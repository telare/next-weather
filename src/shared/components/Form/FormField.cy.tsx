...
export function generateFieldDataCy(
  prefix: string,
  fieldName: string,
  error?: boolean
): string {
  return `[data-cy='${prefix}-form-${fieldName}-field-input${
    error ? "-error" : ""
  }']`;
}
...
//     cy.get(
//       `input${generateFieldDataCy(defaultProps.dataCyPrefix, defaultProps.name)}`
//     ).as("testInput");
//   });
...
//       cy.injectAxe();
//       cy.checkA11y(
//         `p${generateFieldDataCy(defaultProps.dataCyPrefix, defaultProps.name, true)}`
//       );
//     });
//   });
// });
